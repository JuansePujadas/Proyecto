...
def cargar_imagenes():
    ...
    return iniciar_img, opciones_img, puntuaciones_img, salir_img, config_img   ##########

def crear_rectangulos():
    ...
    return rect_iniciar, rect_opciones, rect_puntuaciones, rect_salir, rect_config   ##########

def clic_dentro_rect(rect, pos):    # Función para verificar si un punto está dentro de un rectángulo
    return rect.collidepoint(pos)

def abrir_nueva_ventana(pantalla, color):   # Función para verificar si un punto está dentro de un rectángulo
    pantalla.fill(color)
    pygame.display.flip()
    pygame.time.wait(2000)  # Espera 2 segundos para simular el cambio de ventana

def mostrar_menu(pantalla):   ##########
    iniciar_img, opciones_img, puntuaciones_img, salir_img, config_img = cargar_imagenes()
    rect_iniciar, rect_opciones, rect_puntuaciones, rect_salir, rect_config = crear_rectangulos()
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if clic_dentro_rect(rect_iniciar, pos):
                    abrir_nueva_ventana(PANTALLA,COLOR_ROJO)
                elif clic_dentro_rect(rect_opciones, pos):
                    abrir_nueva_ventana(PANTALLA,COLOR_NEGRO)
                elif clic_dentro_rect(rect_puntuaciones, pos):
                    abrir_nueva_ventana(PANTALLA,COLOR_VERDE)
                elif clic_dentro_rect(rect_config, pos):
                    abrir_nueva_ventana(PANTALLA,COLOR_AZUL)
